\documentclass[11pt]{article}

\usepackage[backend=bibtex]{biblatex}
\usepackage[utf8]{inputenc}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{anysize}
\usepackage{graphicx}
\usepackage{subcaption}
\graphicspath{ {./images/}, {./parts/igor/}, {./parts/igor/images/} }
\usepackage{color}
\usepackage{xcolor}
\usepackage{algorithm2e}
\usepackage{pgfplots}
\usepackage{hyperref}
\usepackage{booktabs}
\usepackage{tabularx}
\usepackage{titlesec}
\usepackage{array}
\usepackage{listings}

\bibliography{bibliography.bib}

\usepackage{tikz}
\usetikzlibrary{shapes,arrows, trees}

\definecolor{mygreen}{rgb}{0,0.6,0}
\definecolor{mygray}{rgb}{0.5,0.5,0.5}
\definecolor{mypurple}{rgb}{0.58,0,0.82}


\usepackage{caption}
\DeclareCaptionFont{white}{\color{white}}
\DeclareCaptionFormat{listing}{\colorbox{gray}{\parbox[c]{\textwidth}{#1#2#3}}}

\setlength\parindent{0pt}
\setlength{\parskip}{10pt}

\marginsize{2cm}{2cm}{1cm}{1cm}
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% % % Commands
\titleformat{\paragraph}
{\normalfont\normalsize\bfseries\space}
{}
{0pt}
{}

% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% % % Document start
\begin{document}

%  Title and authors
   \begin{center}
     {\huge\bfseries B31XP Robotics project\\ Robotic object follower}\\
      \vspace{2ex}
      \textsc{Andrey Pak, Donatas Kozlovskis, Enric Cornellà,\\ Fernando Garcia,  Igor Peric}
   \end{center}
   \vspace{2ex}%

%  Abstract
\begin{abstract}
This project presents a small, easy to build low-cost robot system, that is able to find coloured signs on the floor and implement the associated actions, e.g. stop, turn, pause, etc.
The robot hardware uses Raspberry Pi to control a set of motors, sensors and servo actuators. 
This report provides review of the hardware design and implementation of software using C++ programming language and OpenCV libraries. 
\end{abstract}

\section{Introduction}
\label{sec:intro}

\input{parts/donatas/section_introduction.tex}

\section{Description of the Project}
\input{parts/section_description.tex}


\section{State of the art}

\input{parts/state_art.tex}
 
\section{Project Management}

\input{parts/donatas/section_project_management.tex}

\section{Implementation}
\subsection{Overview}

\input{parts/subsection_overview.tex}

\subsection{Hardware}

\input{parts/subsection_hardware.tex}
	
\subsection{Software}

\input{parts/igor/software-intro.tex}

\subsubsection{State machine}

\input{parts/igor/state-machine.tex}

\subsubsection{Actuators and sensing drivers}
\input{parts/actuators_and_senging_drivers.tex}

\subsubsection{Motion module}
\input{parts/donatas/motion_module.tex}

\subsubsection{Vision module}
\input{parts/igor/vision-module.tex}

\section{ Final setup }
\input{parts/final_setup}

\section{Problems encountered}


\input{parts/section_issues}

\section{Further improvements}
\label{sec:future-improvements}

\input{parts/igor/further-improvements.tex}

\section{Conclusions}

\input{parts/conclusions.tex}
%Example of cite and referencing:\cite{craig2005introduction}

\clearpage
\printbibliography

\end{document}